...
> ![http://xyq.163.com/images/emote/105.gif]({{ "/assets/img/105.gif" | relative_url}})️ In case of "resource class"-based implicit MVC view templates, the controller is also the model. In such case the
...
> ![http://xyq.163.com/images/emote/105.gif]({{ "/assets/img/105.gif" | relative_url}})️ If none or empty template reference is provided (either in `Viewable` or via `@Template`) then the `index`
...
> ![http://xyq.163.com/images/emote/105.gif]({{ "/assets/img/105.gif" | relative_url}})️ Modules that uses capabilities of the base Jersey MVC module register `MvcFeature` automatically, so you don't need
...
> ![http://xyq.163.com/images/emote/105.gif]({{ "/assets/img/105.gif" | relative_url}})️  In a typical set-up, projects using the Jersey MVC templating support would depend on the base module that
